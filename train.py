...
for x in range(6,16):
    ...
    for (classification), group in data.groupby(['class']):
        group.to_csv(f'TRAINC/{x}_{classification}.csv', index=False)
...
for x in classList:
    print(x)
    df = pd.concat(map(pd.read_csv, glob.glob(f'TRAINC/*{x}.csv')),ignore_index=True)
    df["class"].replace(
        {"computer": 1, "standing-up": 2, "standing": 3, "walking": 4, "stairs": 5, "walk-talk": 6, "stand-talk": 7},
        inplace=True)
    df.reset_index(drop=True, inplace=True)
    df.to_csv(f'TRAIN/{x}.csv',index=False)
